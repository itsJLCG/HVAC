...
db.serialize(() => {
  const normalizeQrValues = () => {
    db.run("UPDATE items SET qr_value = name WHERE qr_value IS NULL OR qr_value = '' OR qr_value != name");
  };
...
app.get("/api/items/by-qr/:qr_value", (req, res) => {
  const qrValue = req.params.qr_value;
  db.get("SELECT * FROM items WHERE qr_value = ? COLLATE NOCASE", [qrValue], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
